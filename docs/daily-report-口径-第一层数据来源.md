...
- **覆盖视角**（2026-07-13 起 cover spider 侧日报）：spider 报告代码在 `~/code/python/wechat_spider/tasks/report_task.py`，每日 10:05 生成。口径：已覆盖 = `store_time` 非空（语鲸入库时间由回查任务写回 spider article 表），覆盖时效 = store_time − publish_time 分 ≤1h/1-3h/>3h；未覆盖 = store_time 空（spider 兜底推送），推送成功率 = 未覆盖里 push_result=true 占比（07-12 验证：1-3h 桶 252 篇与 spider 报告完全一致；原"推送成功率暂缺"注记随之闭环）。
